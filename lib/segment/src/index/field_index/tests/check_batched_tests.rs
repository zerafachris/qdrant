...
use common::condition_checker::{ConditionChecker, check_congruence};
...
fn check_index<T: Serialize>(
    ...
    for points in [&points[..0], &points[..]] {
        ...
        for memory in [Memory::Pinned, Memory::Cold] {
            ...
            check_congruence(&mut checker, n, &mut rng);
            ...
            let checker = index.condition_checker(condition, HwMeasurementAcc::new());
            check_congruence(&mut checker.unwrap().unwrap(), n as _, &mut rng);
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                let index = index.unwrap().unwrap();
                let checker = index.condition_checker(condition, HwMeasurementAcc::new());
                check_congruence(&mut checker.unwrap().unwrap(), n, &mut rng);
            }
        }
    }
}
