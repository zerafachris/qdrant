...
#[cfg(test)]
mod tests {
    use common::condition_checker::check_congruence;
    use itertools::izip;
    ...
    fn flat() {
        ...
        for (should, min_should, must, must_not) in izip!(0..=3, 0..=3usize, 0..=3, 0..=3) {
            for min_should_count in 0..=min_should + 1 {
                ...
                check_congruence(&mut filter, 1 << total, &mut rng);
            }
        }
    }
    ...
    fn nested() {
        for seed in 0..20 {
            ...
            check_congruence(&mut filter, 1 << bits, &mut rng);
        }
